...
Teams buy Evidpath when they need more than offline ranking metrics
...
Evidpath is built for questions like:
...
- the recommender implementation lives in `src/evidpath/domains/recommender/`
...
That install exposes both `python -m evidpath` and the
`evidpath` console command. The examples here keep `python -m` for
explicitness.
...
```bash
.venv/bin/python -m evidpath run-swarm --domain recommender --target-url http://127.0.0.1:8051 --brief "test trust collapse and weak first-slate behavior for impatient and exploratory users" --generation-mode fixture --output-dir products/evidpath/output/external-run-swarm-demo
```

Run the lower-level direct audit path against the first external target:

```bash
.venv/bin/python -m evidpath audit --domain recommender --target-url http://127.0.0.1:8051 --scenario returning-user-home-feed --seed 7 --output-dir products/evidpath/output/external-audit-demo
```

Compare the two external services:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-url http://127.0.0.1:8051 --candidate-url http://127.0.0.1:8052 --baseline-label popularity --candidate-label item-item-cf --rerun-count 2 --output-dir products/evidpath/output/external-compare-demo
```

Or let `compare` plan one shared brief-driven coverage bundle for both sides:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-url http://127.0.0.1:8051 --candidate-url http://127.0.0.1:8052 --brief "compare trust collapse and novelty balance across the two systems" --generation-mode fixture --rerun-count 2 --output-dir products/evidpath/output/external-compare-planned-demo
```
...
separate proof package. Evidpath is the product package. Ideas are
...
.venv/bin/python -m evidpath --help
```
...
```bash
.venv/bin/python -m evidpath run-swarm --domain recommender --target-url http://localhost:8010 --brief "test trust collapse and novelty balance for impatient exploratory users" --generation-mode fixture --output-dir products/evidpath/output/run-swarm-demo
```

Run one scenario only:

```bash
.venv/bin/python -m evidpath audit --domain recommender --scenario returning-user-home-feed --seed 7 --reference-artifact-dir products/evidpath/output/reference-artifacts --output-dir products/evidpath/output/demo
```

Run the same audit shape against a real external target:

```bash
.venv/bin/python -m evidpath audit --domain recommender --scenario returning-user-home-feed --seed 7 --target-url http://localhost:8010 --output-dir products/evidpath/output/external-audit-demo
```
...
```bash
.venv/bin/python -m evidpath generate-scenarios --domain recommender --mode fixture --brief "test recommendation quality for sparse-history users who still want novelty" --output-dir products/evidpath/output/generated-scenarios
```
...
```bash
.venv/bin/python -m evidpath plan-run --workflow run-swarm --domain recommender --target-url http://127.0.0.1:8051 --brief "test trust collapse and weak first-slate behavior" --generation-mode fixture --output-dir products/evidpath/output/planned-run
.venv/bin/python -m evidpath execute-plan --run-plan-path products/evidpath/output/planned-run/run_plan.json
```

The same public plan-first flow also works for `compare`:

```bash
.venv/bin/python -m evidpath plan-run --workflow compare --domain recommender --baseline-url http://127.0.0.1:8051 --candidate-url http://127.0.0.1:8052 --brief "compare trust collapse and novelty balance across the two systems" --generation-mode fixture --rerun-count 2 --output-dir products/evidpath/output/planned-compare
.venv/bin/python -m evidpath execute-plan --run-plan-path products/evidpath/output/planned-compare/run_plan.json
```
...
```bash
.venv/bin/python -m evidpath plan-run --workflow audit --domain recommender --target-url http://127.0.0.1:8051 --scenario returning-user-home-feed --output-dir products/evidpath/output/planned-audit
.venv/bin/python -m evidpath execute-plan --run-plan-path products/evidpath/output/planned-audit/run_plan.json
```
...
```bash
.venv/bin/python -m evidpath audit --domain recommender --scenario-pack-path products/evidpath/output/generated-scenarios/provider-pack.json --reference-artifact-dir products/evidpath/output/reference-artifacts --output-dir products/evidpath/output/generated-pack-run
```
...
```bash
.venv/bin/python -m evidpath generate-population --domain recommender --mode fixture --brief "test a broad swarm of novelty-seeking and low-patience viewers" --population-size 12 --output-dir products/evidpath/output/generated-populations
```
...
```bash
.venv/bin/python -m evidpath generate-population --domain recommender --mode provider --ai-profile fast --brief "test a broad swarm of risk-sensitive and exploration-seeking viewers" --population-pack-path products/evidpath/output/generated-populations/provider-population.json
```
...
```bash
.venv/bin/python -m evidpath audit --domain recommender --population-pack-path products/evidpath/output/generated-populations/provider-population.json --reference-artifact-dir products/evidpath/output/reference-artifacts --output-dir products/evidpath/output/generated-population-run
```
...
```bash
.venv/bin/python -m evidpath audit --domain recommender --semantic-mode fixture --reference-artifact-dir products/evidpath/output/reference-artifacts
```
...
```bash
.venv/bin/python -m evidpath audit --domain recommender --semantic-mode provider --semantic-profile fast --reference-artifact-dir products/evidpath/output/reference-artifacts
```

Use the mock fixture explicitly only for narrow testing/debugging:

```bash
.venv/bin/python -m evidpath audit --domain recommender --use-mock
```
...
```bash
.venv/bin/python -m evidpath serve-reference --domain recommender --artifact-dir products/evidpath/output/reference-artifacts --host 127.0.0.1 --port 8020
```

Run compare mode against two artifact-backed targets:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-artifact-dir products/evidpath/output/reference-artifacts-baseline --candidate-artifact-dir products/evidpath/output/reference-artifacts-candidate --rerun-count 3 --output-dir products/evidpath/output/regression-demo
```
...
```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-artifact-dir products/evidpath/output/reference-artifacts-baseline --candidate-url http://localhost:8010 --rerun-count 3 --output-dir products/evidpath/output/regression-demo
```

Reuse one saved population pack across compare reruns:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-artifact-dir products/evidpath/output/reference-artifacts-baseline --candidate-artifact-dir products/evidpath/output/reference-artifacts-candidate --population-pack-path products/evidpath/output/generated-populations/provider-population.json --rerun-count 3 --output-dir products/evidpath/output/regression-demo
```

Add advisory semantic interpretation to compare mode:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-artifact-dir products/evidpath/output/reference-artifacts-baseline --candidate-artifact-dir products/evidpath/output/reference-artifacts-candidate --semantic-mode fixture --rerun-count 3 --output-dir products/evidpath/output/regression-demo
```

Use custom labels in compare mode:

```bash
.venv/bin/python -m evidpath compare --domain recommender --baseline-artifact-dir products/evidpath/output/reference-artifacts-baseline --candidate-artifact-dir products/evidpath/output/reference-artifacts-candidate --baseline-label current-prod --candidate-label next-build
```
...
```bash
.venv/bin/ruff check products/evidpath
.venv/bin/pytest products/evidpath/tests -q
```

## Planning

The active roadmap for this package lives under
[plans/evidpath-v0](../plans/evidpath-v0/README.md).

The current architectural assumptions and subsystem boundaries are summarized in
[plans/evidpath-v0/architecture-assumptions.md](../plans/evidpath-v0/architecture-assumptions.md).

A short component review and the open product decisions are captured in
[plans/evidpath-v0/architecture-review.md](../plans/evidpath-v0/architecture-review.md) and
[plans/evidpath-v0/decision-questions.md](../plans/evidpath-v0/decision-questions.md).

The current in-repo domain plug-in shape is documented in
[plans/evidpath-v0/domain-plugin-architecture.md](../plans/evidpath-v0/domain-plugin-architecture.md).

The foundation-vs-domain ownership rule for future extensions is documented in
[plans/evidpath-v0/foundation-vs-domain-boundaries.md](../plans/evidpath-v0/foundation-vs-domain-boundaries.md).
