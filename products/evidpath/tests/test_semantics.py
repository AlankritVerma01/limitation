...
from evidpath.semantic_interpretation import (
...
def test_fixture_run_semantics_are_structured_and_stable(tmp_path: Path) -> None:
    ...
    assert all(
        explanation.grounding_references for explanation in first.trace_explanations
    )


def test_provider_run_semantics_validate_exact_trace_ids(
    ...
    expected_ids = [
        ...
        for explanation in interpret_run_semantics(
            run_result, mode="fixture"
        ).trace_explanations
    ]
    ...
    with patch(
        "evidpath.semantic_interpretation.request_provider_payload",
        return_value=payload,
    ):
        interpretation = interpret_run_semantics(
            run_result, mode="provider", model_name="gpt-5-mini"
        )
    assert interpretation is not None
    assert interpretation.mode == "provider"
    assert interpretation.model_profile == "custom"
    assert [
        explanation.trace_id for explanation in interpretation.trace_explanations
    ] == expected_ids


def test_provider_run_semantics_reject_malformed_json(
    monkeypatch, tmp_path: Path
) -> None:
    artifact_dir = tmp_path / "artifacts"
    ensure_reference_artifacts(artifact_dir)
...
def test_regression_semantics_explain_only_selected_notable_traces(
    tmp_path: Path,
) -> None:
    ...
    notable_ids = [
        trace_delta["trace_id"] for trace_delta in payload["notable_trace_deltas"][:3]
    ]
