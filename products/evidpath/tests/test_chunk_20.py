...
def test_run_swarm_reuses_both_explicit_packs_without_generating(tmp_path: Path) -> None:
    ...
    with (
        patch("evidpath.orchestration.coverage.generate_scenario_pack") as mock_generate_scenarios,
        patch("evidpath.orchestration.coverage.generate_population_pack") as mock_generate_population,
    ):
        result = main(
            [
                "run-swarm",
