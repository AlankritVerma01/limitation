...
def test_default_policy_warns_on_moderate_metric_regression() -> None:
    ...
    assert any(check.check_id == "metric:mean_session_utility" for check in decision.checks)
...
def test_cohort_override_relaxes_matching_scope_only() -> None:
    ...
    cohort_checks = {check.check_id: check for check in decision.checks if check.check_id.startswith("cohort:")}
    assert cohort_checks["cohort:returning-user-home-feed:Low-patience"].severity == "pass"
    assert cohort_checks["cohort:sparse-history-home-feed:Low-patience"].severity in {"warn", "fail"}
...
def test_same_artifact_dir_regression_passes_and_writes_decision(tmp_path: Path) -> None:
...
def test_module_entrypoint_uses_main_exit_code(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("interaction_harness.cli.main", lambda argv=None: {"exit_code": 1})
    with pytest.raises(SystemExit) as exc_info:
        runpy.run_module("interaction_harness", run_name="__main__")
