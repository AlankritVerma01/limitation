...
def _build_item_payloads(raw_items: dict[str, dict], stats: dict) -> list[dict]:
    ...
    items_payload.sort(key=lambda item: int(item["item_id"]))
...
def _top_neighbors(
    ...
    *,
    ...
    item_positive = max(1, positive_counts[item_id])
    for neighbor_id, shared_count in pair_counts[item_id].items():
        denominator = math.sqrt(item_positive * max(1, positive_counts[neighbor_id]))
        score = shared_count / denominator if denominator else 0.0
        neighbors.append((score, neighbor_id))
    neighbors.sort(reverse=True)
    return [
        {"item_id": neighbor_id, "score": round(score, 6)}
        for score, neighbor_id in neighbors[:limit]
        if score > 0.0
    ]
...
def _expand_preferred_genres(preferred_genres: tuple[str, ...]) -> set[str]:
    ...
    for genre in preferred_genres:
        normalized = _normalize_genre_name(genre)
        expanded.add(normalized)
        expanded.update(PREFERRED_GENRE_ALIASES.get(normalized, ()))
    return expanded
