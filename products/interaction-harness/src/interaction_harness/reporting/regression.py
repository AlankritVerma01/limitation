...
class RegressionMarkdownWriter:
    ...
    def write(self, regression_diff: RegressionDiff, output_dir: Path) -> dict[str, str]:
        ...
        lines = [
            "# Interaction Harness Regression Audit",
            "",
            "## Executive Summary",
            "",
        ...
        for metric in regression_diff.baseline_summary.metric_summaries:
            lines.append(
        ...
        for metric in regression_diff.candidate_summary.metric_summaries:
            lines.append(
                f"| {metric.metric_name} | {metric.mean:.3f} | {metric.minimum:.3f} | {metric.maximum:.3f} | {metric.spread:.3f} |"
            )
...
class RegressionJsonWriter:
    ...
    def write(self, regression_diff: RegressionDiff, output_dir: Path) -> dict[str, str]:
        ...
        payload = _normalize_regression_payload(payload)
